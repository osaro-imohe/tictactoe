...
import {
  StyleSheet, View, Dimensions, TouchableOpacity, Text,
} from 'react-native';
...
const Game = () => {
  const {
    state, setState, incrementPlayerWins, incrementComputerWins, changeIsPlaying,
  ...
  const computersTurn = () => {
    const arr = state.currentGame.slice();
    ...
    if (arr[0][0] == 0 || arr[0][1] == 0 || arr[0][2] == 0 || arr[1][0] == 0 || arr[1][1] == 0 || arr[1][2] == 0 || arr[2][0] == 0 || arr[2][1] == 0 || arr[2][2] == 0) {
      for (let i = 0; i < 2000; ++i) {
        const randomCol = Math.floor(Math.random() * (2 - 0 + 1));
        const randomRow = Math.floor(Math.random() * (2 - 0 + 1));
  ...
  const onBoxPress = (row, col) => {
    ...
    if (playersturn == 1 & checkWinner() === 0) {
      ...
      const nextPlayer = (playersturn === 1) ? -1 : 1;
      setState((prevState) => ({
        ...prevState,
